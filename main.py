...
class RankingSystem(commands.Cog):
    ...
    async def rank(self, interaction: discord.Interaction):
        ...
        embed = discord.Embed(color=Color.TICKET)
        embed.set_author(name=f"{user.display_name} のランクカード", icon_url=user.display_avatar.url)
        embed.set_thumbnail(url=user.display_avatar.url)
        embed.add_field(
            name="🏅 レベル",
            value=f"**Lv.{level}**　(サーバー #{rank_pos})",
            inline=False
        )
        embed.add_field(
            name="✨ XP",
            value=f"`{xp_bar}` {current_xp:,} / {needed_xp:,} XP\n累計: {total_xp:,} XP",
            inline=False
        )
        embed.add_field(
            name="🎙️ VC在室時間",
            value=f"今月: **{fmt_time(month_vc_sec)}**\n累計: {fmt_time(total_vc_sec)}",
            inline=True
        )
        embed.add_field(
            name="💬 メッセージ数",
            value=f"今月: **{month_msgs:,} 件**\n累計: {total_msgs:,} 件",
            inline=True
        )
