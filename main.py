...
logger = logging.getLogger('CestaBank')
...
class BankDatabase:
    def __init__(self, db_path="cesta_bank_v1.db"):
    ...
    def __init__(self, prices: dict):
        ...
        options = [
            discord.SelectOption(
                label="6時間プラン", 
                description=f"{prices.get('6', 5000):,} Ster - ちょっとした作業や会議に", 
                value="6", emoji="🕐"
            ),
            discord.SelectOption(
                label="12時間プラン", 
                description=f"{prices.get('12', 10000):,} Ster - 半日じっくり", 
                value="12", emoji="🕓"
            ),
            discord.SelectOption(
                label="24時間プラン", 
                description=f"{prices.get('24', 30000):,} Ster - 丸一日貸切", 
                value="24", emoji="🕛"
            ),
        ]
    ...
    async def callback(self, interaction: discord.Interaction):
        ...
        async with bot.get_db() as db:
            ...
            if current_bal < price:
                return await interaction.followup.send(f"❌ 残高不足です。\n必要: {price:,} Ster / 所持: {current_bal:,} Ster", ephemeral=True)
    ...
    async def deploy_panel(
        ...
        if description is None:
            description = (
                ...
                f"**⏰ 料金プラン**\n"
                f"• **6時間**: {price_6h:,} Ster\n"
                f"• **12時間**: {price_12h:,} Ster\n"
                f"• **24時間**: {price_24h:,} Ster"
            )
        else:
    ...
    async def confirm(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        async with self.bot.get_db() as db:
            ...
            try:
                ...
                self.stop()
                await interaction.edit_original_response(content=f"✅ {self.receiver.mention} へ {self.amount:,} Ster 送金しました。", embed=None, view=None)
                ...
                try:
                    ...
                    if notify:
                        embed = discord.Embed(title="💰 Ster受取通知", color=discord.Color.green())
                        embed.add_field(name="送金者", value=self.sender.mention, inline=False)
                        embed.add_field(name="受取額", value=f"**{self.amount:,} Ster**", inline=False)
                ...
                if log_ch_id:
                    ...
                    if channel:
                        ...
                        log_embed.add_field(name="金額", value=f"**{self.amount:,} Ster**", inline=True)
                        log_embed.add_field(name="備考", value=self.msg, inline=True)
                        log_embed.add_field(name="処理後残高", value=f"送: {sender_new_bal:,} Ster\n受: {receiver_new_bal:,} Ster", inline=False)
...
class Economy(commands.Cog):
    ...
    async def balance(self, interaction: discord.Interaction, member: Optional[discord.Member] = None):
        ...
        embed = discord.Embed(title="🏛 セスタ銀行 口座照会", color=0xFFD700)
        embed.set_author(name=f"{target.display_name} 様", icon_url=target.display_avatar.url)
        embed.add_field(name="💰 現在の残高", value=f"**{bal:,} Ster**", inline=False)
        embed.set_footer(text=f"Elysion Economy System")
        embed.set_thumbnail(url=target.display_avatar.url)
        
        await interaction.followup.send(embed=embed, ephemeral=True)

    @app_commands.command(name="送金", description="他のユーザーにSterを送金します")
    ...
    async def transfer(self, interaction: discord.Interaction, receiver: discord.Member, amount: int, message: str = "送金"):
        if amount <= 0: return await interaction.response.send_message("❌ 1 Ster 以上を指定してください。", ephemeral=True)
        if amount > 10000000: return await interaction.response.send_message("❌ 1回の送金上限は 10,000,000 Ster です。", ephemeral=True)
        ...
        embed.add_field(name="💰 金額", value=f"**{amount:,} Ster**", inline=True)
    ...
    async def history(self, interaction: discord.Interaction):
        ...
        for r in rows:
            ...
            amount_str = f"{'-' if is_sender else '+'}{r['amount']:,} Ster"
...
class Salary(commands.Cog):
    ...
    def create_salary_slip_embed(self, member, total, matching, month_tag):
        ...
        embed.add_field(name="💵 支給総額", value=f"**{total:,} Ster**", inline=False)
        
        formula = " + ".join([f"{w:,}" for w, r in sorted_matching])
        embed.add_field(name="🧮 計算式", value=f"{formula} = **{total:,} Ster**", inline=False)
        
        breakdown = "\n".join([f"{i+1}. {r.name}: {w:,} Ster" for i, (w, r) in enumerate(sorted_matching)])
    ...
    async def list_wages(self, interaction: discord.Interaction):
        ...
        for row in rows:
            ...
            text += f"{role_str}: **{row['amount']:,} Ster**\n"
    ...
    async def send_salary_log(self, interaction, batch_id, total, count, breakdown, timestamp):
        ...
        embed.add_field(name="総額 / 人数", value=f"**{total:,} Ster** / {count}名", inline=True)
        
        breakdown_text = "\n".join([f"✅ {d['mention']}: {d['amount']:,} Ster ({d['count']}名)" for d in breakdown.values()])
        if breakdown_text:
...
class Jackpot(commands.Cog):
    ...
    async def status(self, interaction: discord.Interaction):
        ...
        embed.add_field(name="💰 現在の賞金総額", value=f"**{pool:,} Ster**", inline=False)
    ...
    @app_commands.command(name="ジャックポット購入", description="ランダムな3桁の番号が付与されます (1枚 5,000 Ster)")
        ...
        async with self.bot.get_db() as db:
            ...
            if len(winners) > 0:
                ...
                if sponsor_id > 0:
                    ...
                    sponsor_msg = f"\n(スポンサー {self.sponsor_name_display} から次回開催費 **{self.seed_money:,} Ster** を徴収しました)"
                else:
                ...
                embed.add_field(name="💰 1人あたりの賞金", value=f"**{prize_per_winner:,} Ster** (手取り)", inline=False)
                ...
                embed.add_field(name="💸 キャリーオーバー", value=f"**{current_pool:,} Ster** は次回に持ち越されます！", inline=False)
...
class Omikuji(commands.Cog):
    ...
    @app_commands.command(name="おみくじ", description="セスタちゃんが今日の運勢を占います (1回 300 Ster)")
    async def omikuji(self, interaction: discord.Interaction):
        ...
        async with self.bot.get_db() as db:
            async with db.execute("SELECT balance FROM accounts WHERE user_id = ?", (user.id,)) as c:
                ...
                if not row or row['balance'] < self.cost:
                    return await interaction.followup.send("セスタ「300Sterすら持ってないの？ 帰って。」", ephemeral=True)
        ...
        res_str = f"**{payout} Ster** (収支: {profit:+d} Ster)"
        if profit < 0:
             res_str += f"\n(💸 負け分の20%はJP賞金へ)"
...
class VoiceSystem(commands.Cog):
    def __init__(self, bot):
        ...
        self.reward_rate = 50 # 基本レート (Ster/分)
    ...
    async def set_vc_rate(self, interaction: discord.Interaction, amount: int):
        ...
        await interaction.response.send_message(f"✅ VC報酬レートを **{amount} Ster / 分** に変更しました。\n(インフレ時は下げ、キャンペーン時は上げてください)", ephemeral=True)
...
class Chinchiro(commands.Cog):
    ...
    def get_cesta_dialogue(self, situation, user_name, amount=0):
    ...
    @app_commands.command(name="チンチロ", description="セスタちゃんと勝負。")
    async def chinchiro(self, interaction: discord.Interaction, bet: int):
        if bet < 100: return await interaction.response.send_message("100Sterから。", ephemeral=True)
        ...
        if last_time:
            ...
            if delta < 3.0: 
                warning_msg = self.get_cesta_dialogue("warning", interaction.user.display_name)
        ...
        if streak >= 6:
            msg = await interaction.response.send_message(f"セスタ「…{streak}連敗中よ？ 頭を冷やしてきなさい。」\n(深呼吸中... ⏳ 5秒)", ephemeral=True)
        ...
        opening_line = self.get_cesta_dialogue("intro", interaction.user.display_name, current_bal)
        ...
        phud = self.render_hud("セスタ", p_dice, p_name, "gold" if p_super else "blue")
    ...
    async def pvp_chinchiro(self, interaction: discord.Interaction, opponent: discord.Member, bet: int):
        ...
        if bet < 500: return await interaction.response.send_message("対戦は500Sterから。", ephemeral=True)
    ...
    async def settle_pve(self, msg, embed, user, bet, multiplier):
        async with self.bot.get_db() as db:
            if multiplier > 0:
                ...
                res_str = f"🎉 **WIN! +{final:,} Ster** (x{multiplier})"
                
                comment_key = "win_big" if multiplier >= 3 else "win"
                comment = self.get_cesta_dialogue(comment_key, user.display_name)
                ...
                res_str = f"💀 **LOSE... -{actual_loss:,} Ster** (x{abs(multiplier)})"
                
                if jackpot_feed > 0:
                     res_str += f"\n(💸 負け額の一部 **{jackpot_feed:,} Ster** がジャックポットへ吸い込まれました...)"
    ...
    @app_commands.command(name="ゴミ拾い", description="所持金が500Ster以下の時だけ使えます。")
    async def scavenge(self, interaction: discord.Interaction):
        async with self.bot.get_db() as db:
            ...
            msg_text = self.get_cesta_dialogue("scavenge", interaction.user.display_name)
            
            if random.randint(1, 20) == 1:
                msg_text = f"「…はぁ。仕方ないわね。\nこれ、私が落としたことにしといてあげる。」\n(セスタがそっぽを向きながら **{amount} Ster** を投げ捨てた！)"

            await interaction.response.send_message(f"{msg_text}\n\n🗑️ 公園で空き缶を拾って **{amount} Ster** になりました。")
...
class Slot(commands.Cog):
    ...
    def get_cesta_comment(self, situation, **kwargs):
    ...
    @app_commands.describe(bet="賭け金 (100 Ster 〜)")
    async def slot(self, interaction: discord.Interaction, bet: int):
        ...
        if last_time and (now - last_time).total_seconds() < 3.5:
            return await interaction.response.send_message("セスタ「目が回るわ…落ち着きなさい。」", ephemeral=True)
        self.last_played[interaction.user.id] = now
        
        streak = self.loss_streak.get(interaction.user.id, 0)
        if streak >= 10:
             await interaction.response.send_message(f"セスタ「…{streak}連敗中よ？ 少し頭を冷やしてきたら？」\n(深呼吸中... ⏳ 5秒)", ephemeral=True)
             await asyncio.sleep(5)
        ...
        try:
            ...
            async with self.bot.get_db() as db:
                async with db.execute("SELECT balance FROM accounts WHERE user_id = ?", (user.id,)) as c:
                    ...
                    if not row or row['balance'] < bet:
                        return await interaction.followup.send("セスタ「お金、足りないみたいよ？ 出直してらっしゃい。」")
            ...
            start_msg = self.get_cesta_comment("start_deep" if is_deep else "start_normal", user=user.display_name)
            if is_ceiling_hit: start_msg = self.get_cesta_comment("ceiling_hit")
            ...
            if is_freeze:
                ...
                if is_respin or is_cesta_save: 
                     disp[1][0] = self.SYMBOLS["MISS"] if is_cesta_save else final_grid[1][0]
                ...
                if is_cesta_save: disp[1][1] = self.SYMBOLS["MISS"]

                is_reach = disp[1][0] == disp[1][1]
                
                if is_reach and not is_cesta_save and random.random() < 0.20:
                    is_cesta_cutin = True

                mid_status = "!!!" if is_reach else "..."
                if is_cesta_cutin: mid_status = "CESTA IS WATCHING..."
                
                mid_color = aura
                if is_reach: mid_color = "red"
                if is_cesta_cutin: mid_color = "pink"
                ...
                if is_cesta_cutin: wait_time = 1.5
                ...
                if is_respin:
                    ...
                    lumen_txt = self.render_slot_screen(miss_grid, "⚡ CESTA PANIC ⚡", "pink")
                    save_msg = self.get_cesta_comment("cesta_save")
                ...
                if is_cesta_save: flash_col = "pink"
            ...
            if multiplier > 0:
                ...
                if is_cesta_save:
                    ...
                    comment = self.get_cesta_comment("win_god")
                    color = 0xffffff
                    res_txt = "**PREMIUM JACKPOT**"
                elif outcome_name in ["SEVEN"]:
                    comment = self.get_cesta_comment("win_big")
                    color = 0xffd700
                    res_txt = "**BIG WIN**"
                elif outcome_name in ["WILD"]:
                    comment = self.get_cesta_comment("win_mid")
                    color = 0xff00ff
                    res_txt = "**SUPER WIN**"
                else:
                    comment = self.get_cesta_comment("win_small")
                    color = 0x00ff00
                    res_txt = "**WIN**"

                if is_ceiling_hit:
                    comment = self.get_cesta_comment("ceiling_hit")
                    res_txt += " (天井到達)"

                embed.clear_fields()
                embed.add_field(name=res_txt if 'res_txt' in locals() else "WIN", value=f"**+{payout:,} Ster**", inline=False)
                embed.color = color
            else:
                charge = int(bet * 0.05)
                if charge > 0:
                ...
                self.loss_streak[user.id] = self.loss_streak.get(user.id, 0) + 1
                comment = self.get_cesta_comment("lose")
                embed.color = 0x2f3136
                embed.clear_fields()
                if charge > 0:
...
class ServerStats(commands.Cog):
    ...
    async def economy_graph(self, interaction: discord.Interaction):
        ...
        try:
            ...
            embed.add_field(name="💵 平均資産", value=f"{int(avg_asset):,} Ster", inline=True)
...
class ShopSelect(discord.ui.Select):
    ...
    async def callback(self, interaction: discord.Interaction):
        ...
        embed.add_field(name="価格", value=f"**{price:,} Ster** / 30日間", inline=False)
...
class ShopSystem(commands.Cog):
    ...
    @has_permission("SUPREME_GOD")
    async def shop_panel(self, interaction: discord.Interaction, shop_id: str, title: str = "🛒 セスタショップ", content: str = "欲しいロールを選択してください！", image_url: str = None):
        await interaction.response.defer()
...
# --- 修正・追加: InterviewSystem (定義されていなかったため、AdminToolsから推測して作成) ---
class InterviewSystem(commands.Cog):
    def __init__(self, bot):
        self.bot = bot

    class InterviewView(discord.ui.View):
        def __init__(self, bot, member, probation_role_id, branch_options):
            super().__init__(timeout=None)
            self.bot = bot
            self.member = member
            ...
            for option in branch_options:
                button = discord.ui.Button(label=option['desc'], emoji=option['emoji'], style=discord.ButtonStyle.secondary)
                button.callback = self.create_callback(option['role_id'])
                self.add_item(button)

        def create_callback(self, role_id_str):
            async def callback(interaction: discord.Interaction):
                await interaction.response.defer()
                role_id = int(role_id_str)
                role = interaction.guild.get_role(role_id)
                probation_role = interaction.guild.get_role(int(self.probation_role_id)) if self.probation_role_id else None
                
                if not role:
                    return await interaction.followup.send("❌ 付与するロールが見つかりません。", ephemeral=True)
                
                try:
                    # 仮ロール削除
                    if probation_role and probation_role in self.member.roles:
                        await self.member.remove_roles(probation_role, reason="面接通過")
                    
                    # 新ロール付与
                    await self.member.add_roles(role, reason="面接通過")
                    ...
                    if log_ch_id:
                        ...
                        if log_ch:
                            embed = discord.Embed(title="🌸 面接合格", description=f"{self.member.mention} が {role.mention} に昇格しました。", color=discord.Color.brand_green())
                            embed.set_author(name=interaction.user.display_name, icon_url=interaction.user.display_avatar.url)
                            await log_ch.send(embed=embed)

                    # メッセージの無効化
                    for child in self.children: child.disabled = True
                    await interaction.message.edit(content=f"✅ {self.member.mention} を {role.mention} に決定しました。", view=self)

                except discord.Forbidden:
                    await interaction.followup.send("❌ 権限不足でロール操作ができませんでした。", ephemeral=True)
                except Exception as e:
                    await interaction.followup.send(f"❌ エラー: {e}", ephemeral=True)
            return callback

    @app_commands.command(name="面接通過", description="【管理者】面接合格者の分岐パネルを指定チャンネルに送信します")
    @app_commands.describe(member="合格したメンバー")
    @has_permission("ADMIN")
    async def pass_interview(self, interaction: discord.Interaction, member: discord.Member):
        await interaction.response.defer(ephemeral=True)
        ...
        async with self.bot.get_db() as db:
            ...
            async with db.execute("SELECT value FROM server_config WHERE key = 'eval_channel_id'") as c:
                row = await c.fetchone()
                target_ch_id = int(row['value']) if row else None
            
            async with db.execute("SELECT value FROM server_config WHERE key = 'probation_role_id'") as c:
                row = await c.fetchone()
                prob_role_id = row['value'] if row else None
            
            # 分岐オプション読み込み
            branch_options = []
            for i in range(1, 6):
                async with db.execute(f"SELECT value FROM server_config WHERE key = 'branch_{i}_role'") as c:
                    r = await c.fetchone()
                    role_id = r['value'] if r else None
                async with db.execute(f"SELECT value FROM server_config WHERE key = 'branch_{i}_emoji'") as c:
                    e = await c.fetchone()
                    emoji = e['value'] if e else "❓"
                async with db.execute(f"SELECT value FROM server_config WHERE key = 'branch_{i}_desc'") as c:
                    d = await c.fetchone()
                    desc = d['value'] if d else f"ルート{i}"
                
                if role_id:
                    branch_options.append({'role_id': role_id, 'emoji': emoji, 'desc': desc})
        
        if not target_ch_id:
            return await interaction.followup.send("❌ 出力先チャンネルが設定されていません。`/評価ログ設定` を実行してください。", ephemeral=True)
        
        target_ch = self.bot.get_channel(target_ch_id)
        if not target_ch:
            return await interaction.followup.send("❌ 出力先チャンネルが見つかりません。", ephemeral=True)

        if not branch_options:
            return await interaction.followup.send("❌ 分岐ルートが設定されていません。`/評価ルート設定` を実行してください。", ephemeral=True)

        embed = discord.Embed(title="🎓 面接結果入力", description=f"対象者: {member.mention}\n\n所属させる部署を選択してください。", color=discord.Color.gold())
        embed.set_thumbnail(url=member.display_avatar.url)
        
        view = self.InterviewView(self.bot, member, prob_role_id, branch_options)
        await target_ch.send(embed=embed, view=view)
        await interaction.followup.send(f"✅ {target_ch.mention} にパネルを送信しました。", ephemeral=True)
...
class CestaBankBot(commands.Bot):
    ...
    async def setup_hook(self):
        ...
        logger.info("CestaBank System: Setup complete and All Cogs Synced.")
    ...
    async def on_ready(self):
        ...
        print("--- Cesta Bank System Online ---")
