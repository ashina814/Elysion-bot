...
class BlackjackView(discord.ui.View):
    ...
    def _embed(self, hide_sesta=True, result_text="", color=0xff00ff):
    ...
    async def hit(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        if p_val > 21:
            ...
            embed = self._embed(hide_sesta=False, result_text=result, color=discord.Color.red())
            await interaction.response.edit_message(embed=embed, view=None)
        else:
            embed = self._embed()
...
class RankingSystem(commands.Cog):
    ...
    @app_commands.command(name="vcランキング", description="今月のVC在室時間ランキングを表示します（全VC対象）")
    @app_commands.describe(top="表示人数（デフォルト10人）")
    async def vc_ranking(self, interaction: discord.Interaction, top: int = 10):
        await interaction.response.defer()
        ...
        async with self.bot.get_db() as db:
            async with db.execute(
                "SELECT user_id, total_seconds FROM vc_rank_stats WHERE month = ? ORDER BY total_seconds DESC LIMIT ?",
                (month_tag, top)
            ) as cursor:
                rows = await cursor.fetchall()
        ...
        medals = ["🥇", "🥈", "🥉"]
        embed = discord.Embed(
            title=f"🎙️ VC在室時間ランキング",
            description=f"集計期間: **{month_tag}** ／ 全VCを対象",
            color=0x5865F2
        )
        ...
        if not rows:
            ...
            for i, row in enumerate(rows):
                ...
                lines.append(f"{rank_label} **{name}** ── {h}時間 {m}分")
